...
from nemi_ai.components.reader.UpstageDocumentParse import (
...
class WeaviateManager:
    def __init__(self):
        self.document_collection_name = "NEMI_DOCUMENTS"
        self.config_collection_name = "NEMI_CONFIGURATION"
        self.suggestion_collection_name = "NEMI_SUGGESTIONS"
    ...
    async def connect(
        ...
        try:
            ...
            if deployment == "Weaviate":
                if weaviateURL == "" and os.environ.get("WEAVIATE_URL_NEMI"):
                    weaviateURL = os.environ.get("WEAVIATE_URL_NEMI")
                if weaviateAPIKey == "" and os.environ.get("WEAVIATE_API_KEY_NEMI"):
                    weaviateAPIKey = os.environ.get("WEAVIATE_API_KEY_NEMI")
                client = await self.connect_to_cluster(weaviateURL, weaviateAPIKey)
            elif deployment == "Docker":
                client = await self.connect_to_docker("weaviate", port)
    ...
    async def verify_cache_collection(self, client: WeaviateAsyncClient, embedder):
        if embedder not in self.embedding_table:
            self.embedding_table[embedder] = "NEMI_Cache_" + re.sub(
                r"[^a-zA-Z0-9]", "_", embedder
            )
            ...
            if embedder.check_available(environment_variables, libraries):
                if "Model" in embedder.config:
                    for _embedder in embedder.config["Model"].values:
                        self.embedding_table[_embedder] = "NEMI_Embedding_" + re.sub(
...
class EmbeddingManager:
    ...
    async def vectorize(
        ...
        @parameter: documents : Document - Nemi-AI document
