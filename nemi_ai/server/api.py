...
production_key = os.environ.get("NEMI_PRODUCTION")
tag = os.environ.get("NEMI_GOOGLE_TAG", "")


if production_key:
    msg.info(f"Nemi-AI runs in {production_key} mode")
    production = production_key
else:
    production = "Local"

manager = nemi_manager.NemiManager()

client_manager = nemi_manager.ClientManager()
...
@app.post("/api/reset")
async def reset_nemi(payload: ResetPayload):
    if production == "Demo":
        return JSONResponse(status_code=200, content={})
    ...
    try:
        ...
        msg.warn(f"Failed to reset Nemi-AI {str(e)}")
