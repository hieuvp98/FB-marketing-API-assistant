...
const StatusMessengerComponent: React.FC<StatusMessengerProps> = ({
  ...
  const getMessageColor = (type: string) => {
    switch (type) {
      case "INFO":
        return "bg-button-nemi";
      case "WARNING":
        return "bg-secondary-nemi";
      case "SUCCESS":
        return "bg-primary-nemi";
      case "ERROR":
        return "bg-warning-nemi";
      default:
        return "bg-button-nemi";
    }
  };
        ...
        {messages
          ...
          .map((message, index) => (
            <motion.div
              ...
              exit={{ opacity: 0, y: 50 }}
              className={`${getMessageColor(message.type)} text-text-nemi p-4 min-w-[300px] rounded-lg z-10 shadow-md`}
            >
