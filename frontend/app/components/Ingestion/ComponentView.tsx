  ...
  return (
    <div className="flex flex-col w-full gap-2">
      <div className="flex gap-2 justify-between">
        <label className="input flex items-center gap-2 w-full bg-bg-nemi">
          <input
            type="text"
        ...
        <button
          ...
          disabled={blocked}
          className="btn flex gap-2 bg-button-nemi border-none hover:bg-secondary-nemi text-text-nemi"
        >
      ...
      <div className="grid grid-cols-3 gap-2">
        {values.map((value, index) => (
          <div
            key={value + index}
            className="flex bg-bg-nemi w-full p-2 text-center text-sm text-text-nemi justify-between items-center rounded-xl"
          >
            ...
            <button
              ...
              className="btn btn-sm btn-square bg-button-nemi border-none hover:bg-warning-nemi text-text-nemi ml-2"
            >
              <FaTrash size={12} />
            </button>
          </div>
  ...
  return (
    <div className="flex flex-col justify-start gap-3 rounded-2xl p-1 w-full ">
      <div className="flex items-center justify-between">
        <div className="divider text-text-alt-nemi flex-grow text-xs lg:text-sm">
      ...
      {!skip_component && (
        <div className="flex flex-col gap-2">
          <div className="flex gap-2 justify-between items-center text-text-nemi">
            ...
            <div className="dropdown dropdown-bottom flex justify-start items-center w-full">
              <button
                ...
                role="button"
                disabled={blocked}
                className="btn bg-button-nemi hover:bg-button-hover-nemi text-text-nemi w-full flex justify-start border-none"
              >
          ...
          <div className="flex gap-2 items-center text-text-nemi">
            <p className="flex min-w-[8vw]"></p>
            <p className="lg:text-sm text-xs text-text-alt-nemi text-start">
              {
                RAGConfig[component_name].components[
                  RAGConfig[component_name].selected
                ].description
      ...
      {Object.entries(
        ...
        <div key={"Configuration" + configTitle + component_name}>
          <div className="flex gap-3 justify-between items-center text-text-nemi lg:text-base text-sm">
            ...
            {typeof config.value != "boolean" &&
              ["text", "number", "password"].includes(config.type) && (
                <label className="input flex text-sm items-center gap-2 w-full bg-bg-nemi">
                  <input
                    type={config.type}
                    className="grow w-full"
                    value={config.value}
            ...
            {typeof config.value != "boolean" &&
              ["textarea"].includes(config.type) && (
                <textarea
                  className="grow w-full text-sm min-h-[152px] bg-bg-nemi rounded-lg p-2"
                  value={config.value}
                  onChange={(e) => {
                    if (!blocked) {
            ...
            {config.type == "bool" && (
              <div className="flex gap-5 justify-start items-center w-full my-4">
                <p className="lg:text-sm text-xs text-text-alt-nemi text-start w-[250px]">
                  {config.description}
                </p>
                <input
                  type="checkbox"
          ...
          {/* Description */}
          {config.type != "bool" && (
            <div className="flex gap-2 items-center text-text-nemi mt-3">
              <p className="flex min-w-[8vw]"></p>
              <p className="text-xs text-text-alt-nemi text-start">
                {config.description}
              </p>
            </div>
          )}
