  ...
  const renderText = (contentSnippet: ContentSnippet, index: number) => {
    if (contentSnippet.type === "text") {
      return (
        ...
        >
          <ReactMarkdown
            className="max-w-[50vw] items-center justify-center flex-wrap prose-sm p-3 prose-pre:bg-bg-alt-nemi"
            components={{
      ...
      return (
        <div
          className="flex p-2 border-2 flex-col gap-2 border-secondary-nemi shadow-lg rounded-3xl"
          ref={contentRef}
        >
          <div className="flex justify-between">
            <div className="flex gap-2">
              <div className="flex gap-2 items-center p-3 bg-secondary-nemi rounded-full w-fit">
                <HiSparkles size={12} />
                <p className="text-xs flex text-text-nemi">Context Used</p>
              </div>
              <div className="flex gap-2 items-center p-3 bg-secondary-nemi rounded-full w-fit">
                <IoNewspaper size={12} />
                <p className="text-xs flex text-text-nemi">
                  Chunk {contentSnippet.chunk_id + 1}
                </p>
              </div>
              {contentSnippet.score > 0 && (
                <div className="flex gap-2 items-center p-3 bg-primary-nemi rounded-full w-fit">
                  <HiSparkles size={12} />
                  <p className="text-xs flex text-text-nemi">High Relevancy</p>
                </div>
              )}
            </div>
          </div>
          <ReactMarkdown
            className="w-full items-center justify-center flex-wrap md:prose-base sm:prose-sm p-3 prose-pre:bg-bg-alt-nemi"
            components={{
              code({ node, inline, className, children, ...props }) {
                const match = /language-(\w+)/.exec(className || "");
                return !inline && match ? (
    ...
    <div className="flex flex-col h-full">
      {document && (
        <div className="bg-bg-alt-nemi flex flex-col rounded-lg overflow-hidden h-full">
          {/* Header */}
          <div className="p-3 bg-bg-alt-nemi">
            <div className="flex gap-4 w-full justify-between">
              <div className="flex gap-4 items-center">
                {isFetching && (
                  <div className="flex items-center justify-center text-text-nemi gap-2">
          ...
          <div className="flex justify-center items-center gap-2 p-3 bg-bg-alt-nemi">
            <NemiButton
            ...
            <div className="flex items-center">
              <p className="text-xs text-text-nemi">
