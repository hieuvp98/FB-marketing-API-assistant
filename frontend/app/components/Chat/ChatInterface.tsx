...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  return (
    <div className="flex flex-col gap-2 w-full">
      ...
      <div className="bg-bg-alt-nemi rounded-2xl flex flex-col h-[50vh] md:h-full w-full overflow-y-auto overflow-x-hidden relative">
        {/* New fixed tab */}
        {selectedSetting == "Chat" && (
          <div className="sticky flex flex-col gap-2 top-0 z-9 p-4 backdrop-blur-sm bg-opacity-30 bg-bg-alt-nemi rounded-lg">
        ...
        >
          <div className="flex w-full justify-start items-center text-text-alt-nemi gap-2">
            {currentDatacount === 0 && <BiError size={15} />}
            {currentDatacount === 0 && (
              <p className="text-text-alt-nemi text-sm items-center flex">{`${currentDatacount} documents embedded by ${currentEmbedding}`}</p>
            )}
          </div>
          ...
          {isFetching.current && (
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-3">
                <span className="text-text-alt-nemi loading loading-dots loading-md"></span>
                <p className="text-text-alt-nemi">
                ...
                <button
                  ...
                  className="btn btn-circle btn-sm bg-bg-alt-nemi hover:bg-warning-nemi hover:text-text-nemi text-text-alt-nemi shadow-none border-none text-sm"
                >
                  <MdCancel size={15} />
                </button>
              </div>
      ...
      <div className="bg-bg-alt-nemi rounded-2xl flex gap-2 p-6 items-center justify-end h-min w-full">
        {socketOnline ? (
          <div className="flex gap-2 items-center justify-end w-full relative">
            <div className="relative w-full">
              <textarea
                className="textarea textarea-bordered w-full bg-bg-nemi placeholder-text-alt-verb min-h min-h-[40px] max-h-[150px] overflow-y-auto"
                placeholder={
                  currentDatacount > 0
              ...
              {currentSuggestions.length > 0 && (
                <ul className="absolute flex gap-2 justify-between top-full left-0 w-full mt-2 z-10 max-h-40 overflow-y-auto">
                  {currentSuggestions.map((suggestion, index) => (
                    <li
                      key={index}
                      className="p-3 bg-button-nemi hover:bg-secondary-nemi text-text-alt-nemi rounded-xl w-full hover:text-text-nemi cursor-pointer"
                      onClick={() => {
          ...
          <div className="flex gap-2 items-center justify-end w-full">
            <button
              onClick={reconnectToNemi}
              className="flex btn border-none text-text-nemi bg-button-nemi hover:bg-button-hover-nemi gap-2 items-center"
            >
